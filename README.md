...
| **8** ✅ done | **8a** ✅ React review dashboard (`frontend/`) — queue view, run detail with full audit trail, decision submission, review history. **8b** ✅ OpenTelemetry tracing + Prometheus metrics + Jaeger/Grafana (`observability/`) — one trace per run spanning the API, Celery, and every agent node; pipeline/task metrics on a provisioned dashboard. **8c** ✅ CI (`.github/workflows/ci.yml`) — lint (`ruff check`) + the offline unit suite on every push/PR to `main`; deliberately excludes integration tests and eval sweeps, which need the live stack, a real LLM, and cost money |
...
| d-0010 | vacant/undeliverable, no forwarding found | `completed` directly, no pause — a confidently vacant address (confidence `1.0`, nothing found) is unambiguous, not uncertain, so `route_after_address` ends the run without a human review; no ask, nothing to print |
...
Note how the three interrupt stages are exercised by different donors: d-0007/d-0009 pause on the address and never reach a major-gift decision (d-0010 is also address-ineligible but, being confidently vacant rather than uncertain, ends `completed` without pausing at all — see its row above), d-0011 sails through address checks and pauses purely on the ask amount, and d-0012 sails through both of those and pauses purely on state solicitation registration — a deterministic legal fact, not a model confidence score, same as the ask-amount gate. Every donor that clears all three review gates continues on through `personalize_letter`, `review_letter_compliance`, and `generate_pdf` — recommendation (0.50 threshold), personalization (0.60), and compliance's own letter-risk review (0.75) are all advisory-gated rather than blocking, so a run can finish `completed` even when one of those confidences was low, or even when `review_letter_compliance` set `approved: false`; each later stage, including PDF generation, runs regardless of the one before it.

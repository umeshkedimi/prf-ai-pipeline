from langgraph.types import interrupt

from app.core.audit import write_audit_log
from app.graph.state import PipelineState

AGENT_NAME = "human_review"


async def human_review(state: PipelineState) -> dict:
    """Pauses the graph via a real LangGraph interrupt() until a decision is
    submitted through POST /workflow/{id}/review. Deliberately nothing runs
    before interrupt() — code placed before it re-executes every time this
    node is resumed until interrupt() actually returns a value (a documented
    LangGraph gotcha), so all side effects (the audit log write) live after."""
    address_result = state.get("address_result") or {}

    decision = interrupt(
        {
            "reason": "address_confidence_below_threshold",
            "address_result": address_result,
            "donor_profile": state.get("donor_profile"),
        }
    )

    updated = dict(address_result)
    action = decision.get("action")
    if action == "modify" and decision.get("updated_address"):
        updated["updated_address"] = decision["updated_address"]
        updated["deliverable"] = True
    elif action == "reject":
        updated["deliverable"] = False
    # "approve": leave address_result exactly as the agent produced it — the
    # human is accepting the existing (low-confidence) assessment as-is, not
    # asserting new certainty about it. Faking a higher confidence number here
    # would misrepresent what actually happened in the audit trail.
    updated["human_reviewed"] = True

    await write_audit_log(
        workflow_run_id=state["workflow_run_id"],
        agent_name=AGENT_NAME,
        step="human_review",
        input_snapshot={"address_result": address_result},
        output=updated,
        reasoning=decision.get("notes"),
        source_refs=[{"reviewer": decision.get("reviewer"), "action": action}],
    )

    return {"address_result": updated, "human_review_decision": decision}
